...
class GroundedAnswerService:
    ...
    def _retrieve_chunks_with_query_expansion_fallback(
        self,
        query: str,
        *,
        doc_ids: list[str] | None = None,
    ) -> tuple[list[RetrievedChunk], float]:
        """Run first-pass retrieval and apply bounded query-expansion fallback when needed."""
        retrieved_chunks, retrieval_seconds = self._retrieve_chunks(
            query,
            doc_ids=doc_ids,
        )

        if not self._should_retry_with_query_expansion(retrieved_chunks):
            return retrieved_chunks, retrieval_seconds
        ...
        merged_chunks = self._merge_unique_chunks(retrieved_chunks)
        for expanded_query in expanded_queries:
            if expanded_query.casefold() == query.casefold():
                continue

            expanded_chunks, extra_seconds = self._retrieve_chunks(
                expanded_query,
                doc_ids=doc_ids,
                top_k=self._retrieval_config.pass2.top_k_per_rewrite,
                rerank_instruction=self._retrieval_config.pass2.rerank_instruction,
            )
            retrieval_seconds += extra_seconds
            merged_chunks = self._merge_unique_chunks([*merged_chunks, *expanded_chunks])
