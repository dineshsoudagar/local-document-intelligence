"""Configuration helpers for Docling parsing and chunk generation."""
...
class ParserConfig:
    ...
    def build_format_options(
        ...
        if InputFormat.PDF in self.allowed_formats:
            options[InputFormat.PDF] = self._build_pdf_format_option(
                enable_picture_description=use_picture_description
            )

        if InputFormat.DOCX in self.allowed_formats:
            options[InputFormat.DOCX] = self._build_docx_format_option()

        if InputFormat.MD in self.allowed_formats:
            options[InputFormat.MD] = self._build_markdown_format_option()

        if InputFormat.HTML in self.allowed_formats:
            options[InputFormat.HTML] = self._build_html_format_option()

        if InputFormat.IMAGE in self.allowed_formats:
            options[InputFormat.IMAGE] = self._build_image_format_option(
                enable_picture_description=use_picture_description
            )
    ...
    def _build_docx_format_option(self) -> Any | None:
        raise NotImplementedError("DOCX parsing is not supported yet.")

    def _build_markdown_format_option(self) -> Any | None:
        raise NotImplementedError("Markdown parsing is not supported yet.")

    def _build_html_format_option(self) -> Any | None:
        raise NotImplementedError("HTML parsing is not supported yet.")
