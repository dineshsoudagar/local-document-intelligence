"""Lightweight FastAPI app used by the packaged bootstrap backend."""
...
async def lifespan(app: FastAPI):
    """Initialize bootstrap-only shared services."""
    ...
    logging.getLogger(__name__).info(
        "Bootstrap lifespan startup app_root=%s code_root=%s runtime_mode=%s "
    ...
    runtime_controller = BootstrapRuntimeController(paths)
    ...
    try:
        ...
        logging.getLogger(__name__).info("Bootstrap lifespan shutdown.")
        runtime_controller.close()


def create_app() -> FastAPI:
    """Create and configure the lightweight bootstrap application."""
    api_config = ApiConfig()
    app = FastAPI(
        title="Offline Document Intelligence Bootstrap API",
        version="0.1.0",
        lifespan=lifespan,
    )
...
def root():
    ...
    return JSONResponse({"message": "Offline Document Intelligence bootstrap API is running."})
